...
local repoFiles = {"ORMS.lua","SaS.lua","gui.lua","ormsLib.lua"}
...
for file,_ in fs.list(installLoc) do
    ...
    if input ~= "n" and input ~= "y" then print("Invalid choice (Y/N)") goto WipeInstDir end
...
for _,file in pairs(repoFiles) do
    if fs.exists(installLoc..file) and not OverwriteAll then
        ...
        ::OverwriteFile::
        local input = string.lower(io.read())
        if input ~= "n" and input ~= "y" and input ~= "a" then print("Invalid choice (Y/N/A)") goto OverwriteFile end
