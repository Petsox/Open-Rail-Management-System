...
local boxNa = component.proxy(component.get("9a29")) -- Signal controller (green/red)
local boxNaVy = component.proxy(component.get("8682")) -- Signal controller (yellow)
...
function SaS.Red(signalName)
  ...
  boxNaVy.setAspect(signalName, 5)
end

function SaS.Yellow(signalName)
  boxNa.setAspect(signalName, 1)
  boxNaVy.setAspect(signalName, 3)
end
