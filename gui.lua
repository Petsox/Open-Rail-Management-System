...
local colorScreenForeground = 0xFFFFFF
local colorTopLineBackground = 0x00FF00
local colorTopLineForeground = 0x000000
local colorBottomLineBackground = 0x00FF00
local colorBottomLineForeground = 0x000000
local colorFrameBackground = 0x000000
	...
	if major > tonumber(versionMajor) then
		...
		if minor > tonumber(versionMinor) then
			...
			gui.newLabel(
				...
				"center",
				5,
				string.format("Installed version is v %d.%d", versionMajor, versionMinor)
			)
			gui.newHLine(compGui, 1, 6, 38)
			gui.newButton(compGui, "center", 7, "exit", gui.exit)
			while true do
...
local function _displayFrame(guiID)
	...
	if guiID.frame == true then
		...
		if guiID.text then
			gpu.set(
				guiID.x + math.floor((guiID.width / 2)) - math.floor((string.len(guiID.text) / 2)),
				guiID.y,
				guiID.text
			)
		end
...
local function _displayAFrame(guiID, frameID)
	if guiID[frameID].visible == true then
		...
		if guiID[frameID].text then
			gpu.set(
				guiID[frameID].x
					+ math.floor((guiID[frameID].width / 2))
					- math.floor((string.len(guiID[frameID].text) / 2) + 1),
				guiID[frameID].y,
				"╡" .. guiID[frameID].text .. "┝"
			)
		end
...
local function wrap(str, limit)
	...
	str:gsub("(%s+)()(%S+)()", function(sp, st, word, fi) -- Function gets called once for every space found.
...
local function _displayList(guiID, listID)
	if guiID[listID].visible == true then
		...
		if guiID[listID].text then
			gpu.set(
				guiID[listID].x + (guiID[listID].width / 2) - (string.len(guiID[listID].text) / 2),
				guiID[listID].y,
				"╡" .. guiID[listID].text .. "┝"
			)
		end
...
local function _displayChart(guiID, chartID)
	if guiID[chartID].visible == true then
		...
		for x = 1, #guiID[chartID].data do
			...
			local dotPos = guiID[chartID].height
				- math.floor(guiID[chartID].height / guiID[chartID].max * guiID[chartID].data[x])
...
function gui.runGui(guiID)
	...
	for i = 1, #guiID do
		...
		elseif guiID[i].type == "list" and guiID[i].enabled == true then
			...
			if
				x > guiID[i].x - 1
				and x < guiID[i].x + guiID[i].width
				and y > guiID[i].y
				and y < guiID[i].y + guiID[i].height - 1
			then
